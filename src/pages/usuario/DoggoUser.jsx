...
const handleDonar = async () => {
  const token = localStorage.getItem("token");
  const monto = prompt("¿Cuánto deseas donar? (en soles)");
  ...
  try {
    ...
    const response = await fetch("http://localhost:8000/donaciones/crear", {
      method: "POST",
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(errorData.detail || "Error al iniciar la donación");
    }

    const data = await response.json();
    window.location.href = data.sandbox_init_point; // Redirige a MercadoPago
  } catch (err) {
    alert(err.message);
  } finally {
    setDonando(false);
